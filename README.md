...
SKS **1.19.1** is the final-hardening release for safe `main` merge and npm publish. It closes the remaining upgrade/publish risks on top of 1.19.0: a 1.18.x→1.19.x **legacy zero-break upgrade** matrix (user `model`/`service_tier`/`model_reasoning_effort` and user-disabled Codex App feature/plugin flags are never overwritten — `sks setup` now seeds those set-if-absent and plugin auto-enable is opt-in via `SKS_MANAGE_CODEX_APP_PLUGINS=1`, matching the install path); a **migration transaction journal** (`.sneakoscope/reports/migration-1.19-journal.jsonl`) recording before/after hashes and backup paths for every config mutation, surfaced by `sks doctor --fix`; a **Zellij launch-command truth** gate plus a real-session **heartbeat-timeout blocker**; a redesigned **Zellij lane UI** (Mission/Mode/Fast/Workers/Codex child · Work · Safety · Blockers · Reports + key-hint footer, width-safe at 80/100/120, NO_COLOR-respecting, middle-ellipsis paths); **packlist + publish-performance** gates that keep the npm tarball to the minimal runtime file set; a **postinstall safe-side-effects** gate (no network/tool-install/process-kill by default); and a **TS source-of-truth / Rust optional-accelerator** boundary gate (publish never compiles Rust; JS fallback always works). The 1.19.0 MAD Zellij dependency repair behavior is unchanged: `sks --mad --yes` installs/repairs required CLI tooling (including Zellij on macOS/Homebrew) before opening the interactive runtime, while npm postinstall never silently mutates Homebrew/npm globals unless `SKS_POSTINSTALL_AUTO_INSTALL_CLI_TOOLS=1` is set.

The new `sks zellij status` / `sks zellij repair` command inspects the Zellij runtime and explains repair steps without auto-installing anything.
