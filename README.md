...
SKS **1.20.3** is a targeted Zellij launch fix for `sks --mad` / codex-lb sessions on macOS: SKS now supplies a short per-user `ZELLIJ_SOCKET_DIR` by default, caps generated session names safely, records `*_command_with_env` attach commands, and classifies `IPC socket path is too long` as `zellij_socket_path_too_long` instead of a generic launch failure.
