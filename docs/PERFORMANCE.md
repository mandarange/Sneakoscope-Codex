...
- The packed package cap is 2488 KiB from the shared release size-budget SSOT, matching sizecheck, publish dry-run performance, and packlist gates while keeping the TypeScript-built `dist` runtime bounded. The build removes non-runtime comments from emitted JavaScript, so the final 7.0.3 host-capability runtime and proof-validation contract measures 2,496,337 packed bytes without relaxing the existing cap; further changes still require measured justification.
