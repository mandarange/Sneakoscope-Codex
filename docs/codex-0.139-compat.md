...
SKS 4.6.1 keeps the historical Codex 0.139 compatibility notes as a fallback baseline while preserving the unified `seo-geo-optimizer` search-visibility route, Lean Engineering Policy evidence, and MadDB direct Supabase MCP `apply_migration` hook fix without restoring legacy split `$SEO`/`$GEO` routes.
