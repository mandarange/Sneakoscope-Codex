...
SKS 4.6.0 keeps the historical Codex 0.139 compatibility notes as a fallback baseline while adding the unified `seo-geo-optimizer` search-visibility route and Lean Engineering Policy evidence: SEO and Generative Engine Optimization share one command surface, mode-specific gates, safe apply/rollback, latest publish tagging, and `npm publish --ignore-scripts` readiness without restoring legacy split `$SEO`/`$GEO` routes.
