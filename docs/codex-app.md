...
SKS 7.0.3 targets the July 16, 2026 stable OpenAI Codex CLI/App baseline `rust-v0.144.5`, with the exact 0.144.5 SDK/CLI dependency graph, active release manifest, and App Server v2 schemas regenerated by that binary kept in lockstep. The `0.145.0-alpha` line is prerelease-only and does not authorize this stable SKS release. Hook output validation uses the vendored OpenAI Codex `latest` generated schemas plus the SKS zero-warning strict subset documented in [codex-cli-compat.md](codex-cli-compat.md). Codex 0.134-0.139 notes remain inherited historical compatibility evidence; they are not the current release baseline.
...
SKS 7.0.3 also reports the active nonsecret auth class. When codex-lb API-key
