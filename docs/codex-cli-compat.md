...
SKS 1.0.5 targets OpenAI Codex CLI `rust-v0.131.0` and treats that tag as the hook/config compatibility baseline. The release gate validates both generated JSON schemas and the runtime semantic rules from `codex-rs/hooks/src/engine/output_parser.rs`.
