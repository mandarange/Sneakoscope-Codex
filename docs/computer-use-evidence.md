...
Generated callout images follow the same local-only default as Chrome Extension or native Computer Use screenshots: SKS may publish metadata, hashes, dimensions, bbox anchors, and Image Voxel relations, but it does not automatically publish screen-content binaries into shared TriWiki.
