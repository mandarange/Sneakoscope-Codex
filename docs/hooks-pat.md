...
SKS hook replay uses the same shared hook policy path as runtime hooks. Fixture replay compares the runtime decision against expected snapshots and redacts token-shaped values with `[redacted]`.
