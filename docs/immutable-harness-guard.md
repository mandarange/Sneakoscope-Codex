...
SKS 1.15.0 makes the immutable harness guard an explicit MAD-SKS release gate.
...
Release readiness expects a protected-core path list, pre/post hash snapshots, write-guard decisions, git diff validation, and proof graph v3 links. If any protected-core file changes during a MAD-SKS run, the route is blocked unless the change is outside MAD-SKS and explicitly handled by normal release engineering.
