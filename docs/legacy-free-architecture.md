...
SKS `0.9.18` keeps the 0.9.14 legacy-free CLI path, removes the route-command monolith, and keeps `src/core/pipeline.mjs` as a compatibility facade for split pipeline modules. `bin/sks.mjs` loads `src/cli/main.mjs`, which loads `src/cli/router.mjs`, which dispatches through `src/cli/command-registry.mjs` to focused wrappers in `src/commands/`. Serious route implementations live in route-specific `src/core/commands/*-command.mjs` modules.
