...
SKS 1.18.9 route flags are runtime inputs, not proof by themselves.
...
A route does not pass because the flags were recorded. It passes only when patch envelopes are generated, central queue artifacts exist, merge coordination groups independent patches, apply results are verified, rollback proof is ready, and `agent-proof-evidence.json` has no patch blockers.
