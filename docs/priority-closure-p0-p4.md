...
SKS 1.18.3 adds route-truth closure for Agent, Team, Research, and QA commands, CLI option propagation into task graph proof, tmux supervisor ownership, slot-lane runtime reuse, and proof-contract reconciliation.
...
| P1 | Codex App/operator UX shows target active slots, total work items, queue counts, backfill counts, generation history, source intelligence, UltraSearch, Codex Web, Goal mode, terminal, and Zellij attach status. |
...
| P4 | Human summaries cover scheduler health, backfill, lane persistence, generation count, source intelligence, UltraSearch proof, Goal official/fallback, terminal close, janitor cleanup, release DAG speedup, and worker Scout local evidence. |
