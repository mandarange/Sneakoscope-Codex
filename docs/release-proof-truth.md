...
SKS 6.1.2 release proof truth distinguishes provider/App Server catalog evidence from live Desktop picker evidence. It requires the package and lockfile to resolve `@openai/codex-sdk` and its `@openai/codex` CLI exactly to 0.144.1, the active manifest target to be `rust-v0.144.1`, and the manifest digest to match schemas generated by that resolved CLI; exact model/effort parity from Naruto work kind through worker intake, SDK config, and proof reports; Voxel TriWiki one-to-one coordinate/voxel validation and bounded wrongness recall; generation-aware retention dry-run/apply evidence; current Codex plugin inventory/repair evidence; new-package menu installation/version proof; duplicate-hook suppression; bundle-id restart completion; and secret-safe Fast status JSON.

Command and retention proof must also show that `sks agent --help` creates no mission, `route_closed` state is not treated as active, and a recently updated non-closed session is protected by the two-hour grace window. New compaction must preserve durable JSON, Image UX/PPT review assets, and unknown evidence byte-for-byte while deleting only the explicit disposable-runtime allowlist. Legacy gzip archives remain readable only when hydration reproduces the original bytes and the SHA-256 in `retention-archive-manifest.json`; storage-budget proof must count every `.sneakoscope` top-level directory and root file under the per-directory 1,000,000-file safety ceiling; release-run retention must leave at most five run directories by default.
...
The current implementation handoff is bounded to typecheck, one clean build, focused changed-surface tests, `release:check:affected`, one `release:check:confidence`, and `npm pack --dry-run --ignore-scripts --json`. That sequence proves release preparation but does not create publish authorization. Publication is a distinct repository-maintainer workflow: the manual main-only OIDC job runs `release:check:full`, verifies the source-bound release stamp, and only then reaches its intentional publish step. The removed `publish:npm` and `release:publish` aliases are historical only and cannot serve as 6.1.2 evidence.
