...
The canonical lifecycle-disabled operator path is `npm run release:check:full`, then `npm run publish:prep-ignore-scripts`, then `npm publish --dry-run --ignore-scripts --json` for verification or `npm run publish:ignore-scripts` for an intentional real publication. The removed `publish:npm` and `release:publish` aliases are historical only and cannot serve as 6.1.0 evidence.
