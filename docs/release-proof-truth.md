...
SKS 4.8.1 release proof truth also requires proof that the macOS menu bar companion presents a visible `SKS` label with a leading status icon instead of relying on an icon-only status item, that temp-home fixtures do not take over the user LaunchAgent label, and that the first-command migration Doctor timeout no longer blocks successful macOS repair profiles.
...
SKS 4.6.3 keeps release proof truth aligned with the 4.6.3 package metadata, Rust helper metadata, built output, release stamp, provenance, npm unpublished-version evidence, InsaneSearch command rename evidence, unified `seo-geo-optimizer` route evidence, MadDB direct `apply_migration` hook evidence, MadDB Supabase transport diagnostics evidence, global npm update detection evidence, Lean Engineering Policy evidence, and publish wrapper evidence before publication.

SKS 4.6.3 release proof truth also requires proof that `sks insane-search` and `$Insane-Search` are the primary source-intelligence command surfaces with UltraSearch kept only as compatibility aliases; that the public split `seo` and `geo` command surfaces are removed; `$SEO-GEO-OPTIMIZER` is the single dollar-command route; crawler/claim/llms.txt gates remain mode-specific; no ranking, traffic, indexing, or AI citation guarantee is claimed; the MadDB direct apply-migration hook gate passes; MadDB Supabase transport diagnostics distinguish read-only transport denial from SQL-plane timeout/interruption/auth failures; `sks update-check` cannot let a newer source checkout mask a stale global npm package; and the latest publish tag avoids downgrading the npm `latest` dist-tag.
