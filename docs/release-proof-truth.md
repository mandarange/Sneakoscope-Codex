...
SKS 4.8.1 release proof truth also requires proof that the macOS menu bar companion presents a visible text-only `SKS` label instead of relying on an icon-only status item, that temp-home fixtures do not take over the user LaunchAgent label, and that the first-command migration Doctor timeout no longer blocks successful macOS repair profiles.
