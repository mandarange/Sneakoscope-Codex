...
SKS 3.0.1 tracks Codex CLI rust-v0.139.0: it adds the `codex:0139-capability` gate and root/mission capability artifacts (code-mode web search, preserved `oneOf`/`allOf` tool schemas, doctor env details, plugin marketplace `source` field with cached catalog, `-P` sandbox profile alias, multi-agent v2 `interrupt_agent` rename) and accepts `interrupt_agent` in cockpit subagent-stage classification. The Codex baseline remains rust-v0.136.0.
