...
SKS 3.1.5 is the Codex App harness type-safe productionization release. It removes `@ts-nocheck` from the core Zellij, doctor, Codex App, loop, and Naruto targets covered by this release; adds typed Codex App probe/report surfaces; makes Zellij self-heal expose dry-run planned mutations; records hook approval and `agent_type` evidence; backs LazyCodex analysis with live source files; preserves directory-local init-deep memory in loop plans; and routes the Codex App execution profile through Loop, QA, Research, and Naruto proof artifacts.
