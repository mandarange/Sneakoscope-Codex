...
SKS 3.1.6 is the Codex-native harness brand-neutrality release. It removes external reference branding from user-visible release surfaces, routes Codex-native feature decisions through a shared broker for Loop, QA, Research, Image, MAD, and Doctor, keeps the Codex 0.139 compatibility baseline explicit, and adds release DAG coverage for brand-neutral leakage, invocation routing, feature broker behavior, reference evidence, init-deep backup hygiene, and doctor readiness UX.
