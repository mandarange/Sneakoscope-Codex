...
SKS 1.21.1 keeps the release-readiness contract stable while fixing three `sks --mad` launch problems. (1) Launch speed: the launch no longer SHA-256-hashes the entire protected core (~1,900 files) on every invocation — the never-compared launch snapshot uses a cheap metadata digest (`snapshotProtectedCore` `mode: 'metadata'`), and the redundant live `codex exec` config probe is skipped on the launch hot path via `runCodexLaunchPreflight`'s `launchFast` (restore with `SKS_LAUNCH_FULL_CODEX_PROBE=1`); the immutable-harness integrity gates keep full content hashing. (2) Zellij clipboard: launched sessions set `copy_command "pbcopy"` + `copy_on_select true` so selections reach the macOS clipboard instead of relying on OSC 52. (3) Codex legacy-profile warning: SKS migrated its config profiles to Codex 0.134+ per-file `$CODEX_HOME/<name>.config.toml` overlays — the project-config splitter drops the deprecated `[profiles.*]` tables / `profile=` selector instead of relocating them into the home config (`[profiles.sks-fast-high]` and `default_profile` are preserved for Codex App fast-mode). It carries forward the 1.21.0 MAD Zellij auto-attach behavior: in an interactive terminal `sks --mad` automatically attaches to the fresh background session, and falls back to printing the exact `Attach with: ZELLIJ_SOCKET_DIR=... zellij attach ...` command when auto-attach is skipped (`--json`, non-TTY, already inside Zellij, `--no-attach`/`SKS_NO_ZELLIJ_ATTACH=1`) or fails. The readiness artifacts surface `zellij_socket_dir`, `zellij_socket_dir_source`, and `*_command_with_env` when Zellij sessions are prepared.

The current `sks.release-readiness.v1` report covers actual Codex config-load truth, Codex config EPERM self-heal, doctor real-fix readiness, MAD launch preflight, Zellij readiness/proof, install-time Zellij dependency repair, Zellij socket-dir launch metadata, MAD attach-command visibility, and official Fast mode service-tier propagation. `ok: true` in the 1.21.1 readiness report means config readability, actual/fake Codex config-load proof, project config policy splitting, EPERM repair proof, MAD preflight, Zellij-only runtime checks, background-layout launch wiring, socket-dir fallback evidence, attach-command output evidence, and `-c service_tier=fast` propagation evidence have no remaining blockers.
...
- `.sneakoscope/reports/release-readiness-1.20.5.json`
- `.sneakoscope/reports/release-readiness-1.20.5.md`
- `.sneakoscope/reports/all-feature-completion-1.20.5.json`
- `.sneakoscope/reports/all-feature-completion-1.20.5.md`
- `.sneakoscope/reports/official-docs-compat-1.20.5.json`
- `.sneakoscope/reports/official-docs-compat-1.20.5.md`
- `.sneakoscope/reports/agent-real-codex-dynamic-smoke-1.20.5.json`
...
- `.sneakoscope/reports/runtime-truth-matrix-1.20.5.json`
