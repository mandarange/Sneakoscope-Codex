...
`sks update` waits out npm's publish propagation (a listed version whose
tarball still returns 404) instead of failing, and an installed SKS reclaims
global skills that a newer, no-longer-active SKS left behind. Verification
must cover the retry ending in success, the retry budget ending in the plain
`not_yet_downloadable` result with nothing installed, the reclaim, and the
refusal that remains when a newer SKS is on PATH.

## Previous candidate: 10.3.5

Published 2026-09-25 as `sneakoscope@10.3.5` from commit 4f5447bf; the
registry `gitHead`, file count (1776), and integrity match the release-check
pack. An update run in the first minutes after that publish hit both failures
10.3.6 fixes.
