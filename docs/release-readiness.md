...
SKS 4.6.3 is a forward patch release carrying the SuperSearch public command rename, the unified SEO/GEO optimizer, Lean Engineering Policy evidence surface, MadDB direct `apply_migration` hook fix, MadDB Supabase transport diagnostics hardening, and global npm update detection hardening. It requires package, lockfile, CLI constants, Rust helper metadata, README, changelog, version-gated release docs, built output, release stamp, provenance, npm registry unpublished-version checks, the `super-search` source-intelligence gate, the `seo-geo-optimizer` mode-specific gate suite, the MadDB direct apply-migration hook gate, the MadDB Supabase transport diagnostics gate, update-check global-install regression coverage, and the `publish:ignore-scripts` wrapper to agree on 4.6.3 before publication.

4.6.3 release readiness adds proof that `sks super-search`, `$Super-Search`, and `$Super-Search` are the primary source-intelligence surface while legacy SuperSearch names remain compatibility aliases; that `sks seo-geo-optimizer` and `$SEO-GEO-OPTIMIZER` are the single public Search Engine Optimization and Generative Engine Optimization surface; that old split `seo`/`geo` public commands are removed; that Lean Engineering Policy evidence reaches planning, workers, code-structure, GPT final review, and Completion Proof; that direct Supabase MCP `apply_migration` is allowed only through the active MadDB capability; that active MadDB can use an explicit write-capable Supabase MCP URL without being shadowed by project-local read-only config; that SQL-plane timeout, interrupted connection, auth, transport, and read-only-denial failures are reported distinctly; that `sks update-check` uses the replaceable global npm install before source checkout versions when deciding updateability; and that `npm publish --ignore-scripts` packages the prebuilt dist surface with the latest dist-tag.

SKS 4.4.0 is the SuperSearch source-intelligence replacement release after 4.2.1. It requires package, lockfile, CLI constants, Rust helper metadata, README, changelog, version-gated release docs, built output, release stamp, provenance, npm registry unpublished-version checks, SuperSearch provider-interface gates, Source Intelligence Policy v2 gates, and the `publish:ignore-scripts` wrapper to agree on 4.4.0 before publication.
...
## Current publish authorization policy (6.2.0)

The 6.2.0 implementation handoff uses this bounded verification sequence:
...
the 6.2.0 command surface; affected selection is owned by the manifest-backed release
DAG.
...
### Maintainer-only publication workflow

The full release DAG is the only command allowed to create publish authorization.
Final operator commands never repair a missing or stale stamp by rerunning the full
release suite. They fail closed and require `npm run release:check:full` to be run as
a separate, reviewable step.

Plain lifecycle-enabled `npm publish` is intentionally blocked by `prepublishOnly`:
npm would otherwise run `prepack` after that authorization point and rebuild the
package that is about to be packed. Maintainer-only local token publication is
supported only through the explicit `publish:prep-ignore-scripts` preflight followed
by `npm publish --ignore-scripts`; that preflight uses npm `whoami`, the package
maintainer list, and a content-bound pack proof, and reports only redacted config
locations. The manual GitHub publish workflow uses npm trusted publishing instead: it
is main-only, requests OIDC identity, runs on Node 24 with npm 11.5.1, and contains no
`NODE_AUTH_TOKEN`, `NPM_TOKEN`, or npmrc token injection. The no-token guarantee is
scoped to that GitHub workflow; it is not a claim about the separate local maintainer
handoff.

Because the maintainer-only final command disables npm lifecycle hooks on purpose,
publication authorization does not rely on `prepublishOnly`. The supported operator
path is `npm run publish:ignore-scripts`; its explicit preflight verifies the current
authoritative full-release stamp before and after package inspection without compiling
again, then checks
dist freshness, version truth, packlist limits, package-script targets, publish tag,
registry version, and the explicitly selected local-token or trusted-publisher authentication mode. It deliberately does not rerun
the serial canonical `npm test`; that proof belongs to the current full-release stamp.

`npm run publish:dry` is a compatibility name for the same preflight followed by
`npm pack --dry-run --ignore-scripts --json`. It never invokes publish dry-run mode.
A raw `npm publish --ignore-scripts` bypasses every npm lifecycle hook by definition,
so it is unsupported unless the explicit repository preflight was just run and its
workspace remained unchanged.

The following long command list and report inventory are archived 1.x/2.x provenance,
not the 6.2.0 release procedure. Some aliases were intentionally removed and must not
...
The following 2.x report names and native-process artifacts are historical examples only; they do not satisfy the 6.2.0 official-subagent gate. Current 6.2.0 proof comes from the canonical subagent plan/events/parent-summary/evidence artifacts and the focused release gates listed at the top of this document.
...
Those historical reports covered version drift, native-process proof, Zellij worker panes, and earlier patch-swarm closure. They remain archival evidence and are not represented as current 6.2.0 completion proof.
...
| P1 | Codex App dashboard/operator visibility for active slots, total work items, pending/active/completed counts, backfill, generation history, source, SuperSearch, Codex Web, Goal, terminal, Zellij physical proof, cleanup status, and work graph score |
...
| P4 | Human-readable summaries for source intelligence, SuperSearch proof, Goal fallback, terminal close, Zellij persistence, physical pane proof, cleanup, scheduler health, and worker Scout evidence |
...
The 6.2.0 SKS menu bar also exposes the current Codex CLI version, an `⬆` update-needed indicator, `Update Codex CLI Now`, and `Run sks doctor --fix`. Read-only status uses `sks codex update-status --json` with a bounded latest-version cache. The explicit mutation action prefers the native `codex update` self-updater; when that command is unavailable, it verifies the selected installation as an official standalone build, npm-global package, or Homebrew cask and invokes only that matching updater. Missing, debug, unsupported, or unverified installations fail closed. The fallback is therefore a visible, provenance-bound global update, not a silent package-manager guess.
