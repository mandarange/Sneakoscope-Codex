...
SKS 1.0.8 writes the `sks.release-readiness.v1` final release seal against the Codex `rust-v0.132.0` compatibility matrix and the UX-Review gpt-image-2 callout/fix loop.
...
- `.sneakoscope/reports/release-readiness-1.0.8.json`
- `.sneakoscope/reports/release-readiness-1.0.8.md`

The report covers version drift, Codex 0.132 structured resume output, source image fidelity metadata, UX-Review generated callout ingestion, text-only fallback blocking, Image Voxel relations, memory summary rebuilds, repeated blocker stops, hook strict subset status, Computer Use evidence mode support, codex-lb persistence truth, docs truthfulness, and remaining P0 gaps. A passing report has no remaining P0 gaps.
...
UX-Review truthfulness is bounded the same way: a real verified UX claim requires a source screenshot, a generated gpt-image-2 annotated callout image, schema-bound issue extraction, bounded safe fixes when requested, and recapture/re-review evidence for changed screens. Mock fixtures remain `verified_partial`, and prose-only screenshot critique cannot pass the gate.
