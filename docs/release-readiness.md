...
SKS 1.21.4 keeps the release-readiness contract stable while making Fast mode state and Naruto clone activity visible in SKS Zellij lanes. The lane renderer now resolves the project-local Fast mode policy when live scheduler/worker artifacts have not recorded `fast_mode` yet, so a Zellij lane can immediately show `Fast  on · service_tier=fast` for the default path or `Fast  off · service_tier=standard` after `sks fast-mode off`. Naruto now prelaunches the right-side Zellij lane stack before scheduler dispatch and uses a less brittle host-capacity model for `codex-exec` concurrency on macOS, with explicit `--concurrency` / `--target-active-slots` and `SKS_NARUTO_MAX_CONCURRENCY` override paths. Interactive Codex panes generated by SKS Zellij layouts now also include `--no-alt-screen`, preserving terminal scrollback so Mac trackpad/wheel gestures scroll the conversation transcript instead of the prompt textarea/history; `SKS_ZELLIJ_CODEX_ALT_SCREEN=1` remains the opt-in alternate-screen escape hatch. SKS carries forward the 1.21.3 Zellij clipboard/mouse-mode launch fix, visual lane count fix, direct-publish stamp repair, Codex Fast mode repair, the 1.21.2 Zellij launch fix, the 1.21.1 launch-speed fix, and the Codex legacy-profile warning fix.
