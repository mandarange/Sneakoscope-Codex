...
SKS 1.21.2 keeps the release-readiness contract stable while fixing the `sks --mad` Zellij launch regression introduced by the 1.21.1 clipboard patch. Zellij 0.44 rejects `--copy-command` when it is paired with the OSC52-only `--copy-clipboard` flag, so launched sessions now pass only `--copy-command pbcopy` and `--copy-on-select true` on the CLI. The generated clipboard config still records `copy_command "pbcopy"`, `copy_clipboard "system"`, and `copy_on_select true` for attach-time config delivery. SKS carries forward the 1.21.1 launch-speed fix (metadata-only interactive launch snapshot plus `runCodexLaunchPreflight` `launchFast`) and Codex legacy-profile warning fix (per-file `$CODEX_HOME/<name>.config.toml` overlays with deprecated project `[profiles.*]` / `profile=` entries dropped).

The current `sks.release-readiness.v1` report covers actual Codex config-load truth, Codex config EPERM self-heal, doctor real-fix readiness, MAD launch preflight, Zellij readiness/proof, install-time Zellij dependency repair, Zellij socket-dir launch metadata, MAD attach-command visibility, and official Fast mode service-tier propagation. `ok: true` in the 1.21.2 readiness report means config readability, actual/fake Codex config-load proof, project config policy splitting, EPERM repair proof, MAD preflight, Zellij-only runtime checks, background-layout launch wiring, socket-dir fallback evidence, attach-command output evidence, and `-c service_tier=fast` propagation evidence have no remaining blockers.
