...
Before `maybeFinalizeRoute` writes proof for a serious route, it ensures the five-scout intake gate has passed. That creates `scout-team-plan.json`, five read-only scout result pairs, `scout-consensus.json`, `scout-handoff.md`, `scout-gate.json`, and `scout-performance.json`, then attaches `evidence.scouts` to Completion Proof.
