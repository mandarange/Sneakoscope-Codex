...
SKS `0.9.13` chooses source-in-package packaging for the Rust accelerator. The npm package includes `crates/sks-core/Cargo.toml`, `Cargo.lock`, and `src/`, but excludes `target/` build output.
