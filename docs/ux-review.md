...
When annotated review images cannot be created or linked, UX-Review may close only as `verified_partial/reference-only`. That closeout requires source screenshots with hashes, docs evidence, source Image Voxel anchors, and Honest Mode evidence; it must not claim generated-image callout extraction, fix verification, or a full production UX review.
