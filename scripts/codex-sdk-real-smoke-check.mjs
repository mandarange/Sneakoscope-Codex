#!/usr/bin/env node
import { assertGate, emitGate, importDist, runFakeCodexSdkTaskFixture } from './lib/codex-sdk-gate-lib.mjs';

const requireReal = process.env.SKS_CODEX_SDK_REQUIRE_REAL === '1' || process.argv.includes('--require-real');
if (!requireReal) {
  ...
  const result = await mod.runCodexTask({
    ...
    cwd: process.cwd(),
    prompt: 'Return JSON matching the requested schema. No file edits.',
    inputFiles: [],
    inputImages: [],
