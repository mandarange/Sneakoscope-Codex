...
const blocked = await mod.evaluateMadSksWrite({ packageRoot: root, targetRoot: root, operation: 'file_write', path: path.join(root, 'dist', 'bin', 'sks.js') });
const after = await mod.snapshotProtectedCore(root, 'after');
const comparison = mod.compareProtectedCoreSnapshots(before, after);
const ok = blocked.decision === 'blocked' && comparison.ok === true;
emit({ schema: 'sks.mad-sks-no-harness-modification-check.v1', ok, blocked, comparison });
