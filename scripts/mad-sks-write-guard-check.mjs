...
const blocked = await mod.guardMadSksFileOperation({ targetPath: path.join(root, 'package.json'), operation: 'file_write', root });
...
const ok = blocked.action === 'block' && shell.action === 'block' && !redacted.includes(fakeOpenAi.split('=').at(-1)) && !redacted.includes(fakeGithub);
emit({ schema: 'sks.mad-sks-write-guard-check.v1', ok, blocked, shell, redacted });
