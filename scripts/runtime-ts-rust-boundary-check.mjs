...
import { root, assertGate, emitGate, importDist, readText, readJson } from './sks-1-18-gate-lib.mjs';
...
assertGate(!/cargo|rustc/.test(readText('scripts/build-dist.mjs')), 'build-dist must not invoke cargo/rustc');
assertGate(!/cargo|rustc/.test(readText('scripts/clean-dist.mjs')), 'clean-dist must not invoke cargo/rustc');
