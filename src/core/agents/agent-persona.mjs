...
export function defaultAgentPersonas(count = 5) {
    return PERSONA_SPECS.slice(0, Math.max(0, count)).map(personaFromSpec);
}
export function validatePersonaUniqueness(personas) {
...
function personaFromSpec(input) {
...
function spec(stableId, role, temperament, riskFocus, allowedTools, readOnly, writePolicy, expectedArtifacts, leasePolicy, completionCriteria) {
    return {
        stableId,
        role,
        temperament,
