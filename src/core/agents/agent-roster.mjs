import { DEFAULT_AGENT_CONCURRENCY, DEFAULT_AGENT_COUNT, MAX_AGENT_COUNT, agentSessionId } from './agent-schema.mjs';
import { defaultAgentPersonas, validatePersonaUniqueness } from './agent-persona.mjs';
import { buildAgentEffortPolicy, decideAgentEffort } from './agent-effort-policy.mjs';
export function normalizeAgentCount(value, fallback = DEFAULT_AGENT_COUNT) {
    const parsed = Number(value ?? fallback);
    if (!Number.isFinite(parsed) || parsed < 1)
        return fallback;
    const count = Math.floor(parsed);
    if (count > MAX_AGENT_COUNT)
        throw new Error('Agent count ' + count + ' exceeds max ' + MAX_AGENT_COUNT);
    return count;
}
export function normalizeAgentConcurrency(value, agents) {
    const parsed = Number(value ?? Math.min(agents, DEFAULT_AGENT_CONCURRENCY));
    if (!Number.isFinite(parsed) || parsed < 1)
        return Math.min(agents, DEFAULT_AGENT_CONCURRENCY);
    if (parsed > MAX_AGENT_COUNT)
        throw new Error('Agent concurrency ' + parsed + ' exceeds max ' + MAX_AGENT_COUNT);
...
export function buildAgentRoster(opts = {}) {
    ...
    if (!uniqueness.ok)
        throw new Error('Invalid agent personas: ' + JSON.stringify(uniqueness));
    const roster = personas.map((persona, index) => {
