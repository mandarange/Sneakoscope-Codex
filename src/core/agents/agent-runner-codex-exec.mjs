...
import { readJson, runProcess, writeJsonAtomic } from '../fsx.mjs';
import { agentWorkerEnv, validateAgentWorkerResult } from './agent-worker-pipeline.mjs';
...
export async function runCodexExecAgent(agent, slice, opts = {}) {
    if (opts.outputSchemaSupported === false) {
        return validateAgentWorkerResult({ mission_id: opts.missionId || opts.mission_id || '', agent_id: agent.id, session_id: agent.session_id, persona_id: agent.persona_id || agent.id, task_slice_id: slice?.id || '', status: 'blocked', backend: 'codex-exec', summary: 'Codex exec output schema support was not verified.', artifacts: [], blockers: ['codex_exec_output_schema_unsupported'], unverified: [], writes: [] });
    }
