import { AGENT_RESULT_SCHEMA, AGENT_WORKER_PIPELINE } from './agent-schema.mjs';
import { scanAgentTextForRecursion } from './agent-recursion-guard.mjs';
import { validateAgentResultSchema } from './agent-output-validator.mjs';
export function agentWorkerEnv(agent, allowedCommandsFile) {
    return {
        SKS_AGENT_WORKER: '1',
...
export function validateAgentWorkerResult(result) {
...
function normalizeLeaseCompliance(value) {
    return {
        ok: value?.ok !== false,
