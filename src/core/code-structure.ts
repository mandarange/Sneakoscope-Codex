...
export async function scanCodeStructure(root: any, opts: any = {}) {
  const files = opts.files?.length ? opts.files.map((file: any) => path.resolve(root, file)) : await listSourceFiles(root);
  const touched = new Set((opts.touchedFiles || []).map((file: any) => path.relative(root, path.resolve(root, file))));
  const entries: any[] = [];
...
function recommendedAction(rel: any, lines: any) {
  ...
  if (/src\/cli\/main\.js$/.test(rel)) return 'extract CLI subcommand handlers into focused modules before adding substantial command logic';
...
function nextSplitCandidate(rel: any) {
  if (/src\/cli\/main\.js$/.test(rel)) return 'goal/wiki/team/eval/db command handlers';
  if (/src\/core\/pipeline\.js$/.test(rel)) return 'route prepare handlers and stop-gate evaluators';
