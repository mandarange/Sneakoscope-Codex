...
export function mapCodexSdkSandboxPolicy(input: CodexTaskInput): { ok: boolean; sandboxMode: CodexSdkSandboxMode; blockers: string[] } {
  ...
  if (input.sandboxPolicy === 'read-only' || contract.read_only === true || contract.sandbox === 'read-only') {
    return { ok: true, sandboxMode: 'read-only', blockers: [] }
  }
  if (input.sandboxPolicy === 'workspace-write') {
    ...
    if (!scoped) return { ok: true, sandboxMode: 'read-only', blockers: [] }
    return {
      ok: true,
