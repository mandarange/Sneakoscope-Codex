  ...
  if (action === 'build') {
    if (!flag(args, '--mock')) {
      ...
      if (!imagegenRequired.ok) {
        ...
        console.error('PPT build blocked: the active SKS image mode could not generate an image (see `sks imagegen status --json`).');
...
async function pptImagegenReview(root: string, command: any, action: string, args: any[] = []) {
  ...
  if (!mockMode) {
    ...
    if (!imagegenRequired.ok) {
      ...
      process.exitCode = 1;
      if (flag(args, '--json')) return printJson(result);
      console.error('PPT imagegen review blocked: the active SKS image mode could not generate an image (see `sks imagegen status --json`).');
      for (const action of imagegenRequired.blocker?.next_actions || []) console.error(`- ${action}`);
      return result;
    }
  }
...
function nextActionForPptGate(gate: any = {}) {
  ...
  if (blockers.includes('ppt_imagegen_callouts_missing')) return 'Generate slide callout review images with `sks imagegen generate --reference <slide image>` and rerun extraction.';
