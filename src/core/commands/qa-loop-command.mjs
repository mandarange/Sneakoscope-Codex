...
export async function qaLoopCommand(sub, args = []) {
    ...
    if (action === 'prepare')
        return qaLoopPrepare(actionArgs);
    if (action === 'answer')
        return qaLoopAnswer(actionArgs);
    if (action === 'run')
...
async function qaLoopPrepare(args) {
    ...
    if (!prompt)
        throw new Error('Missing QA target prompt.');
    ...
    if (flag(args, '--json'))
        return console.log(JSON.stringify({ schema: 'sks.qa-loop-prepare.v1', ok: true, mission_id: id, questions_required: true, native_agent_plan: nativeAgentPlan }, null, 2));
    console.log(`QA-LOOP mission created: ${id}`);
...
async function qaLoopAnswer(args) {
    ...
    if (!id || !answerFile)
        throw new Error('Usage: sks qa-loop answer <mission-id|latest> <answers.json>');
...
async function qaLoopRun(args) {
    ...
    if (flag(args, '--json'))
        return console.log(JSON.stringify({ schema: 'sks.qa-loop-run.v1', ok: false, mission_id: id, gate, proof: proof.validation }, null, 2));
...
async function qaLoopStatus(args) {
    ...
    const id = await resolveMissionId(root, args[0]);
    if (!id)
        throw new Error('Usage: sks qa-loop status <mission-id|latest>');
    const { dir, mission } = await loadMission(root, id);
