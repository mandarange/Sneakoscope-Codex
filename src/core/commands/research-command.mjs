...
export async function researchCommand(sub, args = []) {
    if (sub === 'prepare')
        return researchPrepare(args);
    if (sub === 'run')
        return researchRun(args);
    if (sub === 'status')
        return researchStatus(args);
...
async function researchRun(args) {
    ...
    if (!id)
        throw new Error('Usage: sks research run <mission-id|latest> [--mock] [--max-cycles N] [--cycle-timeout-minutes N]');
...
async function researchStatus(args) {
    ...
    const id = await resolveMissionId(root, args[0]);
    if (!id)
        throw new Error('Usage: sks research status <mission-id|latest>');
    const { dir, mission } = await loadMission(root, id);
...
async function researchCodeMutationSnapshot(root, missionId = null) {
    ...
    if (tracked.code !== 0 || status.code !== 0)
        return { ok: false, reason: 'git_unavailable', hashes: {}, status_rows: [], error: tracked.stderr || status.stderr };
    const allowedPrefixes = researchAllowedMutationPrefixes(missionId);
    const hashes = {};
    for (const rel of tracked.stdout.split(/\r?\n/).map((line) => line.trim()).filter(Boolean)) {
        if (researchMutationAllowedPath(rel, allowedPrefixes))
            continue;
...
async function researchCodeMutationDelta(root, baseline, missionId) {
    if (!baseline?.ok)
        return { blocked: false, changed_paths: [], reason: baseline?.reason || 'baseline_unavailable' };
    const current = await researchCodeMutationSnapshot(root, missionId);
    if (!current.ok)
        return { blocked: false, changed_paths: [], reason: current.reason || 'current_snapshot_unavailable' };
    const changed = new Set();
    for (const [rel, hash] of Object.entries(current.hashes)) {
        if (baseline.hashes[rel] !== hash)
            changed.add(rel);
    }
    for (const rel of Object.keys(baseline.hashes)) {
        if (!(rel in current.hashes))
            changed.add(rel);
    }
    const baselineRows = new Set(baseline.status_rows || []);
    for (const row of current.status_rows || []) {
        if (baselineRows.has(row))
            continue;
        const rel = porcelainStatusPath(row);
        if (rel && !researchMutationAllowedPath(rel, current.allowed_prefixes))
            changed.add(rel);
    }
