...
import { exists, projectRoot, runProcess, writeJsonAtomic } from '../fsx.mjs';
...
async function runSks(root, commandArgs) {
  ...
  const sourceBin = new URL('../../../bin/sks.mjs', import.meta.url).pathname;
...
function routeExecutionResult(route, command, result, options = {}) {
  ...
  return {
    schema: 'sks.run-route-execution.v1',
    ok,
    status: ok ? options.okStatus || 'verified_partial' : 'blocked',
    route: route.command,
    command,
