...
export function buildAllFeaturesSelftest(registry: any, opts: any = {}): JsonData {
  ...
  const checks = [
    ...
    checkRow('agent_lease_policy_present', registry.features.some((feature: any) => feature.id === 'route-native-agent-intake' && /read-only/i.test(JSON.stringify(feature.contract || {})) && /lease/i.test(JSON.stringify(feature.contract || {}))), ['route-native-agent-intake']),
