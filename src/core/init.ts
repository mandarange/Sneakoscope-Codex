...
export async function initProject(root: any, opts: any = {}) {
  ...
  const manifest: any = {
    ...
    database_safety: 'default_safe; $MAD-SKS live-full-access profile is centralized in src/core/permission-gates.js and keeps only catastrophic DB safeguards',
  ...
  function defaultPolicy(scope: any, commandPrefix: any) {
    return {
      ...
      database_safety: {
        ...
        mad_sks_gate_module: 'src/core/permission-gates.js'
      },
      performance: {
...
export async function installSkills(root: any) {
  const imageUxReviewSkill = (name: any) => `---\nname: ${name}\ndescription: $Image-UX-Review/$UX-Review imagegen/gpt-image-2 annotated UI/UX review loop.\n---\n\nUse only for $Image-UX-Review, $UX-Review, $visual-review, or $ui-ux-review UI/UX review requests. ${imageUxReviewPipelinePolicyText()} ${CODEX_IMAGEGEN_REQUIRED_POLICY} Core loop: capture or attach source UI screenshots, then invoke Codex App $imagegen with gpt-image-2 to create a new generated annotated review image from each source screenshot, then analyze the generated review image with vision/OCR into image-ux-issue-ledger.json, then apply only requested safe fixes and recheck changed screens. Text-only screenshot critique cannot satisfy full verification; missing generated annotated review images keep full image-ux-review-gate.json verification blocked, but may close verified_partial/reference-only when source screenshots plus hashes, docs evidence, source Image Voxel anchors, and Honest Mode evidence exist. Use Codex Computer Use for live UI/browser capture when available; browser automation screenshots are not a substitute for Codex Computer Use evidence. Required artifacts: image-ux-review-policy.json, image-ux-screen-inventory.json, image-ux-generated-review-ledger.json, image-ux-issue-ledger.json, image-ux-iteration-report.json, image-ux-review-gate.json. Finish with reflection and Honest Mode.\n`;
  const skills = {
