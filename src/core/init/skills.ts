...
async function installOfficialSkills(root: any) {
  ...
  const skills = {
    ...
    'swarm': `---\nname: swarm\ndescription: Deprecated $Swarm compatibility alias for the $Naruto Codex official subagent workflow.\n---\n\nWarn once that $Swarm is a deprecated alias, then follow the naruto skill. Use Codex official subagents and agent threads through \`sks naruto run "<task>" [--agents N] [--max-threads N] [--json]\`. The parent runs on GPT-5.6 Sol Max, clear bounded worker slices use GPT-5.6 Luna Max, and reasoning-sensitive expert slices use GPT-5.6 Sol Max. Wait for all requested subagents, keep write slices disjoint, accept official SubagentStart/SubagentStop event evidence, and keep only subagent-plan.json, subagent-events.jsonl, subagent-evidence.json, verification-summary.json, and naruto-summary.json by default. Finish with scoped verification, reflection when required, and Honest Mode.\n`,
    ...
    'team': `---\nname: team\ndescription: Deprecated $Team compatibility alias; new execution missions use the $Naruto Codex official subagent workflow.\n---\n\nWarn once that $Team and \`sks team "<task>"\` are deprecated for new execution, then redirect implementation to \`sks naruto run "<task>" [--agents N] [--max-threads N] [--json]\`. Legacy \`sks team log|tail|watch|lane|status|event|message|open-zellij|attach-zellij|cleanup-zellij\` remains read-only observability for old missions only. New work uses a GPT-5.6 Sol Max parent, Luna Max worker agents for clear bounded slices, Sol Max expert agents for judgment work, official SubagentStart/SubagentStop evidence, and the five-artifact Naruto budget. ${leanEngineeringCompactText()} ${outcomeRubricPolicyText()} ${speedLanePolicyText()} ${solutionScoutPolicyText('fix this broken behavior')} ${skillDreamPolicyText()}\n`,
    ...
    'naruto': `---\nname: naruto\ndescription: $Naruto is the SKS alias for a Codex official subagent workflow using official agent threads.\n---\n\nUse when the user explicitly invokes $Naruto or requests bounded parallel delegation. Prefer \`sks naruto run "<task>" [--agents N] [--max-threads N] [--json]\`, plus \`sks naruto status|subagents|proof [--mission <id>] [--json]\`. The parent runs on GPT-5.6 Sol Max. Use the worker custom agent on GPT-5.6 Luna Max for clear, bounded, repeatable slices and the expert custom agent on GPT-5.6 Sol Max for UI, review, debugging, planning, strategy, architecture, integration, security, DB, release, ambiguity, or any judgment-sensitive slice. Codex [agents].max_threads is the concurrency authority (project default 12) and max_depth=1 forbids recursive delegation. Split work into independent outcomes, avoid duplicate assignments and overlapping writes, state when a slice is read-only, wait for every requested subagent, and let the parent own integration and final judgment. Accept official SubagentStart/SubagentStop events as runtime evidence; native process/PID proof is not required. Keep the default artifact budget to subagent-plan.json, subagent-events.jsonl, subagent-evidence.json, verification-summary.json, and naruto-summary.json. Refresh/validate TriWiki only when the selected work profile requires it, run scoped verification that can change the decision, then finish with reflection when required and Honest Mode.\n`,
    'shadow-clone': `---\nname: shadow-clone\ndescription: Deprecated $ShadowClone compatibility alias for the $Naruto Codex official subagent workflow.\n---\n\nEmit one deprecation warning, translate the request to $Naruto, and use \`sks naruto run "<task>" [--agents N] [--max-threads N] [--json]\`. Do not describe the new default as clones, a process swarm, or a native kernel. Follow the naruto skill's Sol Max parent, Luna Max worker, Sol Max expert, official event evidence, max_depth=1, five-artifact budget, scoped verification, reflection, and Honest Mode rules.\n`,
...
function isPrunableGeneratedPath(rel: any) {
  ...
  // Agent files are compatibility/user-extension surfaces. A file that drops
  // out of the new default inventory must be preserved instead of interpreted
  // as stale, because legacy and user-authored roles intentionally coexist.
