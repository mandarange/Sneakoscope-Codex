...
async function installOfficialSkills(root: any) {
  ...
  const skills = {
    ...
    'sks': `---\nname: sks\ndescription: General Sneakoscope Codex command route for $SKS or $sks usage, setup, status, and workflow help.\n---\n\nUse local SKS commands: bootstrap, deps, commands, quickstart, codex-app, context7, guard, conflicts, reasoning, wiki, pipeline status, pipeline plan, skill-dream. Promote code-changing work to Naruto unless Answer/DFix/Help/Wiki/safety route fits. Surface route/guard/scope, use TriWiki, do not edit installed harness files outside this engine repo, and quarantine OMX/DCodex conflicts via update/setup/doctor --fix or \`sks conflicts cleanup --yes\`. ${skillDreamPolicyText()}\n`,
