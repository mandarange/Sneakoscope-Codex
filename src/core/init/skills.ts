...
async function installOfficialSkills(root: any) {
  ...
  const skills = {
    ...
    'imagegen': `---\nname: imagegen\ndescription: Generate or edit rasters with the current GPT Image model.\n---\n\nUse for image generation/editing, UX annotated review images, and PPT raster assets. ${CODEX_IMAGEGEN_REQUIRED_POLICY} Model documentation: ${OPENAI_IMAGEGEN_MODEL_DOC_URL}. API request documentation: ${OPENAI_IMAGE_GENERATION_DOC_URL}. Codex host capabilities: ${CODEX_APP_IMAGE_GENERATION_DOC_URL}. Read the official model catalog and image guide before choosing a newer model; reconcile the shared imagegen-model-policy.ts and request validation when the documented contract changes. Do not equate a file in generated_images or a model name in a prompt with evidence of the engine used. Preserve original references and record the selected provider, requested model, completed output path, hash, and any returned model identity. ${IMAGEGEN_SOCIAL_SOURCE_POLICY} Follow design.md when relevant.\n`,
    'imagegen-source-scout': `---\nname: imagegen-source-scout\ndescription: Verify the latest GPT Image model from official OpenAI docs.\n---\n\nRead ${OPENAI_IMAGEGEN_CATALOG_DOC_URL}, then the current model page (${OPENAI_IMAGEGEN_MODEL_DOC_URL}) and ${OPENAI_IMAGE_GENERATION_DOC_URL}. Check ${CODEX_APP_IMAGE_GENERATION_DOC_URL} separately for host model selection; API availability does not prove the built-in Codex tool uses that model. Report the exact recommended undated model ID, API model field, supported parameters, host restrictions, and documentation date. There is no assumed evergreen GPT Image alias. Use these facts to update the shared policy, never a parallel model list. ${IMAGEGEN_SOCIAL_SOURCE_POLICY} Do not generate images in this skill.\n`,
