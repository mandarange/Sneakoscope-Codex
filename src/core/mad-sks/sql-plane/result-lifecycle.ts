import { extractCanonicalToolCallId, transitionMadSksSqlPlaneOperation } from './operation-store.js';

export interface MadSksSqlPlaneLifecycleHook {
  mission_id: string;
  operation_id: string;
...
export async function recordMadSksSqlPlaneToolResult(input: {
  root: string;
  missionId: string;
  hook: MadSksSqlPlaneLifecycleHook;
  ok: boolean;
  ...
  if (!input.hook.tool_call_id) {
    return {
      schema: 'sks.mad-sks-sql-plane-tool-result-lifecycle.v2',
      ok: false,
      skipped: true,
  ...
  const operation = await transitionMadSksSqlPlaneOperation({
    root: input.root,
...
export async function maybeRecordMadSksSqlPlaneToolResultFromToolUse(input: {
  root: string;
  missionId: string;
  toolCallPayload?: any;
  toolResult?: any;
  ...
  const ok = !madSksSqlPlaneToolUseFailed(payload);
  return recordMadSksSqlPlaneToolResult({
    root: input.root,
    missionId: input.missionId,
    hook: hook || {
...
export function lifecycleHookFromUnknown(value: any): MadSksSqlPlaneLifecycleHook | null {
  const candidate = value?.ledger_result_hook || value?.mad_sks_sql_plane?.ledger_result_hook || value;
...
function madSksSqlPlaneToolUseFailed(payload: any = {}) {
