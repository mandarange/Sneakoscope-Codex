...
export interface MadSksSqlPlaneRuntimeProfile {
...
export async function createMadSksSqlPlaneRuntimeProfile(input: {
  root: string;
  missionId: string;
  cycleId: string;
  projectRef: string;
  ...
  mcpUrl?: string | null;
}): Promise<MadSksSqlPlaneRuntimeProfile> {
  ...
  const profilePath = path.join(dir, 'codex-mad-sks-sql-plane.config.toml');
  ...
  const profile: MadSksSqlPlaneRuntimeProfile = {
    schema: 'sks.mad-sks-sql-plane-runtime-profile.v1',
...
export async function closeMadSksSqlPlaneRuntimeProfile(input: { root: string; missionId: string; profile?: MadSksSqlPlaneRuntimeProfile | null; reason?: string }): Promise<ReadOnlyRestorationProof> {
  const runtimeDir = madSksSqlPlaneRuntimeDir(input.root, input.missionId);
  ...
  await writeJsonAtomic(path.join(runtimeDir, 'read-only-restoration.json'), {
    ...proof,
...
export function redactedRuntimeProfile(profile: MadSksSqlPlaneRuntimeProfile): Omit<MadSksSqlPlaneRuntimeProfile, 'server_url'> {
  const { server_url: _serverUrl, ...rest } = profile;
  return rest;
}

export function madSksSqlPlaneMcpUrl(projectRef: string, explicitUrl?: string | null): string {
  if (explicitUrl) {
