...
export function qaNativeAgentPlan(input = {}) {
  ...
  return {
    ...
    personas: QA_NATIVE_AGENT_PERSONAS.map((persona) => ({
      ...persona,
      session_id: input.missionId ? `${input.missionId}-${persona.id}` : `${persona.id}-session`,
      outputs: (persona.outputs || []).map((artifact) => artifact === 'qa-report.md' ? reportFile : artifact)
    })),
...
export async function writeQaNativeAgentLedger(dir, input = {}) {
  ...
  const root = await initializeAgentCentralLedger(dir, {
    ...
    roster: {
      ...
      roster: plan.personas.map((persona) => ({
        id: persona.id,
    ...
    partition: {
      ...
      leases: plan.batches.flatMap((batch) => batch.outputs.map((artifact) => ({
        path: artifact,
        owner_agent_id: batch.agents[0],
  ...
  for (const batch of plan.batches) {
    await appendAgentLedgerEvent(root, {
      agent_id: batch.agents[0],
      session_id: `${missionId}-${batch.agents[0]}`,
