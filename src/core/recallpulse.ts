...
export function validateResearchScoutPersonas(scoutLedger: any = {}, geniusSummaryText: any = '') {
  const rows = Array.isArray(scoutLedger?.scouts) ? scoutLedger.scouts : [];
  const issues: any[] = [];
  const byId = new Map(RESEARCH_SCOUT_PERSONA_CONTRACT.map((scout: any) => [scout.id, scout]));
