...
export const COMMAND_CATALOG = [
  ...
  { name: 'agent', usage: 'sks agent run|status|close|cleanup <mission-id|latest> [--agents N] [--work-items N] [--target-active-slots N] [--mock] [--apply|--dry-run] [--drain] [--stale-ms N] [--json]', description: 'Run, inspect, close, or clean native multi-session agent missions with agents as target active slots, work items as the route queue size, and cleanup executor proof for stale runtime resources.' },
