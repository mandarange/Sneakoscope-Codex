...
import { nowIso, readJson, runProcess } from '../../fsx.mjs';
...
export async function readCodexAppSubagentCapability(file = process.env.SKS_CODEX_APP_SUBAGENTS_CAPABILITY_FILE) {
...
export function validateCodexAppSubagentCapability(descriptor = null) {
  const blockers = [];
...
export async function runCodexAppSubagentEngine(root, { missionId, dir, route, task, roles } = {}) {
  ...
  if (!capability.available) {
    return {
      ...
      jobs: jobs.map((job) => ({ ...job, status: 'rejected', code: 127, reason: capability.blockers.join('; ') }))
    };
  }
  const [cmd, ...baseArgs] = capability.descriptor.launch_command;
  const runJobs = await Promise.allSettled(roles.map(async (role, index) => {
  ...
  return {
    ...
    jobs: runJobs.map((entry) => entry.status === 'fulfilled' ? entry.value : { status: 'rejected', reason: entry.reason?.message || String(entry.reason) })
  };
}
