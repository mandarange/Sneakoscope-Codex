...
import { SCOUT_CONSENSUS_SCHEMA, SCOUT_COUNT, SCOUT_RESULT_SCHEMA } from './scout-schema.mjs';
import { scoutRouteLabel } from './scout-plan.mjs';
export function buildScoutConsensus({ missionId = null, route = '$Team', results = [], parallelMode = 'parallel', generatedAt = nowIso() } = {}) {
    const invalidResults = results.flatMap((result) => scoutResultSchemaBlockers(result).map((issue) => `${result?.scout_id || 'unknown'}:${issue}`));
    const consensusResults = results.filter((result) => scoutResultSchemaBlockers(result).length === 0 && result.status === 'done');
    ...
    const unverified = [
        ...results.flatMap((result) => result.unverified || []),
        ...(invalidResults.length ? ['Schema-invalid scout results were excluded from consensus/proof promotion.'] : [])
    ];
    const findings = consensusResults.flatMap((result) => result.findings || []);
    const suggested = consensusResults.flatMap((result) => result.suggested_tasks || []);
    const wrongnessReferences = [...new Set(consensusResults.flatMap((result) => result.wrongness_references || []))];
    const activeAvoidanceRules = dedupeRules(consensusResults.flatMap((result) => result.wrongness_context?.active_avoidance_rules || []));
    ...
    const dbRequired = consensusResults.some((result) => result.scout_id === 'scout-3-safety-db' && result.findings?.some((finding) => finding.kind === 'db' || finding.kind === 'risk'));
    const visualRequired = consensusResults.some((result) => result.scout_id === 'scout-4-visual-voxel' && result.required_image_voxel_evidence?.length);
    return {
        ...
        context7: {
            required: consensusResults.some((result) => result.context7_required === true),
            libraries: [...new Set(consensusResults.flatMap((result) => result.context7_libraries || []))]
        },
...
function summarizeSourcePolicy(results = [], { rejected = 0 } = {}) {
    const counts = {
...
export function scoutResultSchemaBlockers(result = {}) {
    const blockers = [];
    if (result.schema !== SCOUT_RESULT_SCHEMA)
        blockers.push(`invalid_schema:${result.schema || 'missing'}`);
    if (result.status !== 'done')
        blockers.push(`not_done:${result.status || 'missing'}`);
    if (result.schema_validation?.ok === false)
        blockers.push('schema_validation_failed');
    if (Array.isArray(result.parse_issues) && result.parse_issues.length)
        blockers.push('parse_issues_present');
    if (result.read_only !== true || result.read_only_confirmed !== true)
...
export function renderScoutHandoff(consensus = {}) {
    ...
    for (const slice of consensus.implementation_slices || []) {
        ...
        if (slice.verification?.length)
            lines.push(`  Verification: ${slice.verification.join(', ')}`);
    }
    lines.push('', '## Required Tests', '');
    for (const command of consensus.required_tests || [])
        lines.push(`- ${command}`);
    lines.push('', '## Proof Evidence', '');
    for (const item of consensus.required_proof_evidence || [])
        lines.push(`- ${item}`);
    if (consensus.wrongness_references?.length) {
        lines.push('', '## Wrongness References', '');
        for (const id of consensus.wrongness_references)
            lines.push(`- ${id}`);
    }
    if (consensus.blockers?.length) {
        lines.push('', '## Blockers', '');
        for (const blocker of consensus.blockers)
            lines.push(`- ${blocker}`);
    }
    if (consensus.unverified?.length) {
        lines.push('', '## Unverified', '');
        for (const item of consensus.unverified)
            lines.push(`- ${item}`);
    }
    return `${lines.join('\n')}\n`;
}
function riskFromFiles(files = []) {
    if (files.some((file) => /db|sql|migration|permission|auth/i.test(file)))
        return 'high';
    if (files.length > 4)
        return 'medium';
    return 'low';
}
//# sourceMappingURL=scout-consensus.js.map