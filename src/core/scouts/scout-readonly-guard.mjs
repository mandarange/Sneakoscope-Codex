...
import { listFilesRecursive, rel, runProcess, sha256 } from '../fsx.mjs';
...
export async function snapshotScoutReadableTree(root, { missionId } = {}) {
    ...
    for (const file of files) {
        ...
        if (isVolatileRuntimePath(relative))
            continue;
        if (isAllowedScoutWrite(relative, missionId))
            continue;
...
function isVolatileRuntimePath(relativePath) {
    ...
    if (/^\.sneakoscope\/state\//.test(relPath))
        return true;
    if (/^\.sneakoscope\/.*\.tmp$/.test(relPath))
        return true;
    return false;
}
export async function assertScoutReadOnly(root, before, { missionId } = {}) {
    ...
    for (const relative of [...paths].sort()) {
        ...
        if (!prev && next)
            violations.push({ path: relative, kind: 'added' });
        else if (prev && !next)
            violations.push({ path: relative, kind: 'removed' });
        else if (prev && next && prev.sha256 !== next.sha256)
...
export function isAllowedScoutWrite(relativePath, missionId) {
    ...
    if (!relPath)
        return false;
    if (isAmbientMissionRuntimePath(relPath, missionId))
        return true;
    if (missionId && relPath.startsWith(`.sneakoscope/missions/${missionId}/`)) {
        ...
        if (name.startsWith('scout-benchmarks/'))
            return true;
    ...
    if (/^\.sneakoscope\/reports\/scout-[^/]+\.(json|md|jsonl)$/.test(relPath))
        return true;
    if (/^\.sneakoscope\/wiki\/wrongness-(index|ledger|summary)\.(json|md)$/.test(relPath))
        return true;
    return false;
}
function isAmbientMissionRuntimePath(relPath, missionId) {
    if (!missionId)
        return false;
...
async function gitStatusSnapshot(root, { missionId } = {}) {
...
function gitStatusDelta(before = {}, after = {}, missionId = null) {
    const beforeMap = new Map((before?.entries || []).map((row) => [row.raw, row]));
    const afterRows = after?.entries || [];
    const changed = afterRows.filter((row) => !beforeMap.has(row.raw));
    const allowed = changed.filter((row) => row.allowed || isAllowedScoutWrite(row.path, missionId));
    const disallowed = changed.filter((row) => !row.allowed && !isAllowedScoutWrite(row.path, missionId));
    return {
...
//# sourceMappingURL=scout-readonly-guard.js.map
