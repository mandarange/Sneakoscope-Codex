...
test('official prompt seals model, ownership, wait, and no-nesting rules', () => {
  ...
  assert.ok(prompt.includes(`model: ${T.fast}/low`))
  assert.ok(prompt.includes(`model: ${T.deep}/max`))
  assert.doesNotMatch(prompt, /luna_max_mechanical|sol_max_judgment/)
...
test('official prompt teaches capacity-derived automatic fan-out and the hard ceiling', () => {
  ...
  assert.match(prompt, /initial targets are 4\/6\/8 \(16 for mass fast\/context work\) and may grow after decomposition/)
