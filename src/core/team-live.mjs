...
export function parseTeamSpecArgs(args = []) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (/^--(?:agents|sessions|team-size)$/.test(arg)) {
            ...
            if (!explicitExecutor)
                roleCounts.executor = explicitSession;
            i++;
            continue;
        }
...
export function normalizeTeamRoleCounts(input = {}) {
    ...
    for (const [key, value] of Object.entries(input || {})) {
        ...
        if (role)
            counts[role] = normalizeTeamAgentSessions(value, counts[role] || 1);
    }
...
export function buildTeamRoster(roleCounts = DEFAULT_TEAM_ROLE_COUNTS, opts = {}) {
    ...
    return {
        ...
        analysis_team: nativeAgents.map((agent) => ({ ...agent, write_policy: 'read-only native analysis', output: 'team-analysis.md' })),
...
async function reconcileTeamTmuxFromEvent(dir, record = {}) {
    if (!process.env.TMUX || String(process.env.SKS_TMUX_EVENT_RECONCILE || '1') === '0')
        return null;
    if (record.type === 'tmux_lane_opened')
        return null;
    ...
    if (!cockpitState?.missions?.[missionId])
        return null;
