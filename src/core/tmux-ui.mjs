...
import { EMPTY_CODEX_INFO, getCodexInfo } from './codex-adapter.mjs';
...
function codexArgsWithFastServiceTier(args = []) {
    ...
    for (let i = 0; i < input.length; i += 1) {
        ...
        if ((arg === '-c' || arg === '--config') && /^service_tier\s*=/.test(next.trim()))
            return input;
        if (/^(?:-c|--config)=service_tier\s*=/.test(arg.trim()))
            return input;
    }
...
export function platformTmuxInstallHint() {
    if (process.platform === 'darwin')
        return 'Install tmux 3.x or newer: brew install tmux';
...
export function tmuxReadinessCatchFallback(err) {
...
export async function tmuxReadiness(opts = {}) {
    ...
    if (bin && !version) {
        ...
        if (run.code === 0)
            version = parseTmuxVersion(run.stdout || run.stderr || '');
        else
            error = run.stderr || run.stdout || 'tmux -V failed';
    }
...
export function codexLaunchCommand(root, codexBin, codexArgs = [], launchEnv = {}) {
...
export function teamLaneStyle(agentId = '') {
    ...
    if (!id || id === 'mission_overview' || id === 'overview')
        return TMUX_TEAM_LANE_STYLES.overview;
    if (/analysis|agent/.test(id))
        return TMUX_TEAM_LANE_STYLES.agent;
    if (/debate|consensus|planner|user/.test(id))
        return TMUX_TEAM_LANE_STYLES.planning;
    if (/db|safety/.test(id))
        return TMUX_TEAM_LANE_STYLES.safety;
    if (/review|qa|validation/.test(id))
        return TMUX_TEAM_LANE_STYLES.review;
    if (/executor|implementation|worker|developer/.test(id))
        return TMUX_TEAM_LANE_STYLES.execution;
    return TMUX_TEAM_LANE_STYLES.planning;
}
...
export function teamAgentCommand(root, missionId, agentId, phase, _promptFile = null) {
    const style = teamLaneStyle(agentId);
...
export function formatTmuxBanner(status = null) {
    ...
    if (status)
        lines.push('', formatCodexAppStatus(status));
    return lines.join('\n');
}
...
async function currentTmuxTarget(bin, env = process.env) {
    if (!isTmuxShellSession(env))
        return { ok: false, reason: 'not running inside tmux' };
    const run = await tmuxRun(bin, ['display-message', '-p', '#{session_name}\t#{window_id}\t#{pane_id}'], { timeoutMs: 5000, maxOutputBytes: 4096 });
    if (run.code !== 0)
        return { ok: false, reason: run.stderr || run.stdout || 'tmux display-message failed' };
    const [session, windowId, paneId] = String(run.stdout || '').trim().split('\t');
    if (!session || !windowId)
        return { ok: false, reason: 'tmux did not report current session/window' };
...
async function isRecordedSksTmuxSession(root, session) {
    ...
    if (!record)
        return { ok: false, reason: 'current tmux session is not recorded as an SKS session' };
    const recordRoot = path.resolve(record.root || root || process.cwd());
    const resolvedRoot = path.resolve(root || process.cwd());
    if (recordRoot !== resolvedRoot)
        return { ok: false, reason: `recorded SKS session belongs to ${recordRoot}` };
...
function teamCockpitAgentIds(plan = {}, dashboard = null, control = null, opts = {}) {
    ...
    if (!agents)
        return opts.plannedFallback ? visible : [];
    ...
    if (concrete.length)
        return uniqueAgentIds(concrete);
    const active = [];
    for (const id of visible) {
        ...
        if (!status || status === 'pending')
            continue;
        if (isTerminalTeamAgentStatus(status))
            continue;
        active.push(id);
    }
    if (!active.length && opts.plannedFallback)
        return visible;
...
async function listTmuxWindowPanes(bin, windowId) {
    ...
    if (run.code !== 0)
        return { ok: false, panes: [], stderr: run.stderr || run.stdout || 'tmux list-panes failed' };
...
async function listAllTmuxPanes(bin) {
    ...
    if (run.code !== 0)
        return { ok: false, panes: [], stderr: run.stderr || run.stdout || 'tmux list-panes -a failed' };
...
async function listAllTmuxSessions(bin) {
    ...
    if (run.code !== 0)
        return { ok: false, sessions: [], stderr: run.stderr || run.stdout || 'tmux list-sessions failed' };
    return { ok: true, sessions: parseTmuxSessionLines(run.stdout) };
}
async function tmuxPaneExists(bin, paneId) {
    if (!paneId || !String(paneId).startsWith('%'))
        return false;
...
async function setTmuxPaneUserOptions(bin, paneId, options = {}) {
    ...
    for (const [key, value] of Object.entries(options)) {
        ...
        if (run.code === 0)
            ...
            failed.push({ command, stderr: run.stderr || run.stdout || 'tmux set-option failed' });
    }
    return { applied, failed };
}
async function writeTmuxCockpitRecord(root, record = {}) {
    if (!record.mission_id || !record.session)
        return null;
...
async function readRecordedTeamSurfaceHints(root) {
    ...
    for (const record of records) {
        ...
        for (const pane of Array.isArray(record.panes) ? record.panes : []) {
            if (session && pane?.pane_id)
                paneKeys.add(`${session}\t${pane.pane_id}`);
        }
    }
    const missionDirs = await fsp.readdir(missionRoot).catch(() => []);
    for (const name of missionDirs) {
        if (!/^M-\d{8}-\d{6}-[A-Za-z0-9]+$/.test(name))
            continue;
        missionIds.add(name);
...
function paneBelongsToKeptMission(pane = {}, keepMissionIds = new Set()) {
    if (pane.mission_id && keepMissionIds.has(pane.mission_id))
        return true;
...
function shouldSweepTeamPane(pane = {}, hints = {}, opts = {}) {
    if (!pane?.pane_id || pane.pane_id === opts.currentPaneId)
        return false;
    if (paneBelongsToKeptMission(pane, opts.keepMissionIds || new Set()))
        return false;
    const missionIds = hints.missionIds || new Set();
    const sessions = hints.sessions || new Set();
    const paneKeys = hints.paneKeys || new Set();
    if (pane.managed && pane.mission_id && missionIds.has(pane.mission_id))
        return true;
    if (paneKeys.has(`${pane.session}\t${pane.pane_id}`))
        return true;
    if (sessions.has(pane.session) && isLegacyTeamPane(pane))
        return true;
...
function teamMainPaneWidthFromWindow(width) {
    ...
    if (!Number.isFinite(n) || n <= 0)
        return TEAM_TMUX_MAIN_PANE_MIN_WIDTH;
...
async function applyStableTeamLayout(tmuxBin, target, mainPaneId = null, opts = {}) {
    ...
    const runAndRecord = async (args) => {
        ...
        if (run.code === 0)
            ...
            failed.push({ command, stderr: run.stderr || run.stdout || 'tmux command failed' });
        return run;
    };
    if (mainPaneId)
        await runAndRecord(['select-pane', '-t', mainPaneId]);
...
export function shouldAutoAttachTmux(args = [], env = process.env, streams = {}) {
    ...
    if (args.includes('--json') || args.includes('--status-only') || args.includes('--quiet') || args.includes('--no-attach'))
        return false;
    if (String(env.SKS_TMUX_NO_AUTO_ATTACH || '') === '1')
        return false;
...
export async function createTmuxSession(plan = {}, panes = [], opts = {}) {
    ...
    if (await hasTmuxSession(tmuxBin, session)) {
        if (opts.recreate || opts.replaceExisting) {
            ...
            if (killed.code !== 0)
                return { ok: false, session, panes: [], stderr: killed.stderr || killed.stdout || 'tmux kill-session failed' };
        }
        else {
    ...
    if (create.code !== 0)
        return { ok: false, session, panes: [], stderr: create.stderr || create.stdout || 'tmux new-session failed' };
    ...
    for (const pane of normalizedPanes.slice(1)) {
        ...
        if (split.code !== 0)
            return { ok: false, session, panes: created, stderr: split.stderr || split.stdout || 'tmux split-window failed' };
        const newPaneId = paneId(split.stdout);
        if (newPaneId && !(await tmuxPaneExists(tmuxBin, newPaneId)))
            return { ok: false, session, panes: created, stderr: `tmux split-window returned pane ${newPaneId}, but the pane was not present after creation` };
        created.push({ pane_id: newPaneId, role: pane.role || 'lane', title: pane.title || null });
        if (rightSidePanes && !rightStackRootPaneId && newPaneId)
            rightStackRootPaneId = newPaneId;
        if (!rightSidePanes)
...
export async function launchTmuxUi(args = [], opts = {}) {
    ...
    if (created.ok)
        await writeTmuxSessionRecord(plan.root, { session: created.session, attach_command: created.attach_command, panes: created.panes, mode: opts.codexLbBypassed ? 'codex_lb_bypassed_openai_session' : opts.codexLbFreshSession ? 'codex_lb_fresh_session' : 'codex_session' }).catch(() => null);
    if (!args.includes('--quiet')) {
        console.log(`SKS tmux session: ${created.session || plan.session}`);
        if (codexLbSweep?.closed_session_count)
            console.log(`codex-lb tmux cleanup: closed ${codexLbSweep.closed_session_count} stale detached session(s)`);
        else if (codexLbSweep && !codexLbSweep.ok && !codexLbSweep.skipped)
            console.log(`codex-lb tmux cleanup: skipped (${codexLbSweep.reason || 'tmux cleanup failed'})`);
        if (created.ok && created.reused)
            console.log('tmux: reused existing session');
        else if (created.ok)
            console.log(`tmux: created ${created.panes.length} pane(s)`);
        else
            console.log(`tmux: not created (${created.stderr || 'tmux failed'})`);
        if (created.ok)
            console.log(`Attach: ${created.attach_command}`);
    }
    let attached = null;
...
export async function launchMadTmuxUi(args = [], opts = {}) {
    ...
    if (created.ok)
        await writeTmuxSessionRecord(plan.root, { session: created.session, attach_command: created.attach_command, panes: created.panes, mode: opts.codexLbBypassed ? 'mad_codex_lb_bypassed_openai_session' : opts.codexLbFreshSession ? 'mad_codex_lb_fresh_session' : 'mad_session', mission_id: missionId }).catch(() => null);
    if (!args.includes('--quiet')) {
        ...
        if (codexLbSweep?.closed_session_count)
            ...
            console.log(`codex-lb tmux cleanup: skipped (${codexLbSweep.reason || 'tmux cleanup failed'})`);
        if (created.ok)
            console.log(`tmux: opened ${created.panes.length} pane(s)`);
        else
            console.log(`tmux: not created (${created.stderr || 'tmux failed'})`);
        if (created.ok)
            console.log(`Attach: ${created.attach_command}`);
...
function printTmuxLaunchBlocked(plan, opts = {}) {
    if (opts.concise) {
        ...
        if (!plan.tmux.ok)
            console.error(`- tmux missing: ${platformTmuxInstallHint()}`);
        if (!plan.codex.bin)
            console.error('- Codex CLI missing. Install: npm i -g @openai/codex@latest, or set SKS_CODEX_BIN.');
        return;
    }
    console.log(formatTmuxBanner(plan.app));
    console.log('\nLaunch blocked:\n');
    for (const blocker of Array.from(new Set(plan.blockers)))
        console.log(`- ${blocker}`);
}
...
export async function reconcileTmuxTeamCockpit({ root, missionId, plan = {}, promptFile = null, dashboard = undefined, control = undefined, close = false, plannedFallback = false, env = process.env, tmux = null } = {}) {
    ...
    if (String(env.SKS_TMUX_DYNAMIC_TEAM || '1') === '0')
        return { ok: false, skipped: true, reason: 'SKS_TMUX_DYNAMIC_TEAM=0' };
    ...
    if (!ownership.ok)
        return { ok: false, skipped: true, session: target.session, reason: ownership.reason };
    ...
    if (!paneList.ok)
        return { ok: false, skipped: false, session: target.session, window_id: target.window_id, reason: paneList.stderr };
    const cockpitState = await readJson(tmuxCockpitStatePath(resolvedRoot), {}).catch(() => ({}));
    ...
    const closePane = async (pane, reason) => {
        ...
        if (kill.code === 0)
            closed.push({ pane_id: pane.pane_id, agent: pane.agent, role: pane.role, mission_id: pane.mission_id, reason });
        else
            failed.push({ action: 'kill-pane', pane_id: pane.pane_id, agent: pane.agent, mission_id: pane.mission_id, reason, stderr: kill.stderr || kill.stdout || 'tmux kill-pane failed' });
    };
    ...
    for (const pane of remainingManaged) {
        if (pane.agent && !byAgent.has(pane.agent))
            byAgent.set(pane.agent, pane);
    }
    let rightStackRootPaneId = remainingManaged[0]?.pane_id || null;
    for (const lane of lanes) {
        if (byAgent.has(lane.agent))
            continue;
        ...
        if (!rightStackRootPaneId)
            rightStackRootPaneId = pane_id;
...
export async function sweepTmuxTeamSurfaces({ root, keepMissionId = null, env = process.env, tmux = null, dryRun = false } = {}) {
    ...
    if (!paneList.ok)
        return { ok: false, skipped: true, reason: paneList.stderr, closed_lane_count: 0, targets: [] };
    ...
    for (const pane of targets) {
        ...
        const kill = await tmuxRun(tmuxBin, ['kill-pane', '-t', pane.pane_id], { timeoutMs: 5000 });
        if (kill.code === 0)
            closed.push({ pane_id: pane.pane_id, session: pane.session, window_id: pane.window_id, agent: pane.agent, role: pane.role, mission_id: pane.mission_id, title: pane.title });
        else
            failed.push({ pane_id: pane.pane_id, session: pane.session, window_id: pane.window_id, agent: pane.agent, mission_id: pane.mission_id, title: pane.title, stderr: kill.stderr || kill.stdout || 'tmux kill-pane failed' });
    }
...
export async function sweepCodexLbTmuxSessions({ root, keepSession = null, env = process.env, tmux = null, dryRun = false } = {}) {
    ...
    if (!sessionList.ok)
        return { ok: false, skipped: true, reason: sessionList.stderr, closed_session_count: 0, targets: [] };
    const current = await currentTmuxTarget(tmuxBin, env).catch(() => ({ ok: false }));
    ...
    for (const target of targets) {
        ...
        if (kill.code === 0)
            closed.push({ session: target.session, created_at: target.created_at, last_attached_at: target.last_attached_at });
        else
            failed.push({ session: target.session, stderr: kill.stderr || kill.stdout || 'tmux kill-session failed' });
    }
    return {
        ok: failed.length === 0,
        skipped: false,
            ...
            if (cockpit.opened?.length) {
                ...
                if (await exists(dir)) {
                    for (const lane of cockpit.opened) {
                        if (!lane.agent || lane.agent === 'mission_overview')
                            continue;
        ...
        if (current.ok && current.session === session) {
            ...
            if (panes.ok) {
                for (const pane of panes.panes.filter((entry) => entry.pane_id !== current.pane_id && isLegacyTeamPane(entry))) {
                    const kill = await tmuxRun(tmuxBin, ['kill-pane', '-t', pane.pane_id], { timeoutMs: 5000 });
                    if (kill.code === 0)
                        closed.push({ pane_id: pane.pane_id, title: pane.title });
                    else
                        failed.push({ pane_id: pane.pane_id, title: pane.title, stderr: kill.stderr || kill.stdout || 'tmux kill-pane failed' });
                }
            }
        }
        else {
            ...
            killed_session = kill.code === 0;
            if (!killed_session)
                failed.push({ session, stderr: kill.stderr || kill.stdout || 'tmux kill-session failed' });
        }
    }
    if (current.ok) {
        const panes = await listTmuxWindowPanes(tmuxBin, current.window_id);
        if (panes.ok) {
            for (const pane of panes.panes.filter((entry) => !entry.managed && entry.pane_id !== current.pane_id && isLegacyTeamPane(entry))) {
                if (closed.some((entry) => entry.pane_id === pane.pane_id))
                    continue;
                const kill = await tmuxRun(tmuxBin, ['kill-pane', '-t', pane.pane_id], { timeoutMs: 5000 });
                if (kill.code === 0)
                    closed.push({ pane_id: pane.pane_id, title: pane.title });
...
async function cleanupRecordedTmuxTeamPanes(root, missionId, record = {}) {
    ...
    if (!target)
        return { ok: false, skipped: true, reason: 'no recorded tmux target', closed_lane_count: 0 };
    const tmuxBin = await findTmuxBin() || 'tmux';
    const paneList = await listTmuxWindowPanes(tmuxBin, target);
    if (!paneList.ok)
        return { ok: false, skipped: true, reason: paneList.stderr, closed_lane_count: 0 };
    ...
    for (const pane of targets) {
        const kill = await tmuxRun(tmuxBin, ['kill-pane', '-t', pane.pane_id], { timeoutMs: 5000 });
        if (kill.code === 0)
            closed.push({ pane_id: pane.pane_id, agent: pane.agent, role: pane.role });
        else
            failed.push({ pane_id: pane.pane_id, agent: pane.agent, stderr: kill.stderr || kill.stdout || 'tmux kill-pane failed' });
    }
    if (closed.length) {
...
//# sourceMappingURL=tmux-ui.js.map