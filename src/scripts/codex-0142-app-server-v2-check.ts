...
const requireReal = process.argv.includes('--require-real') || process.env.SKS_REQUIRE_CODEX_0142_APP_SERVER === '1';
const clientSource = readText('src/core/codex-control/codex-app-server-v2-client.ts');
const schema = readText('schemas/codex/app-server-0.142/codex_app_server_protocol.v2.schemas.json');
...
assertGate(schema.includes('"thread/list"'), 'generated 0.142 schema must contain thread/list');
assertGate(schema.includes('"thread/read"'), 'generated 0.142 schema must contain thread/read');
assertGate(schema.includes('"searchTerm"'), 'generated 0.142 schema must contain ThreadListParams.searchTerm');
assertGate(schema.includes('"ThreadSearchResult"'), 'generated 0.142 schema must contain ThreadSearchResult');
...
if (requireReal) {
  const { client, runtimeIdentity } = await createCodexAppServerV2Client({ requestedBy: 'codex-0142-app-server-v2-check' });
  try {
    ...
    assertGate(runtimeIdentity.version === '0.142.0', 'app-server-v2 require-real must resolve Codex 0.142.0', realProbe);
...
emitGate('codex:0142:app-server-v2', {
