...
const resolved = await resolveCodexRuntime({ requestedBy: 'codex-0142-binary-identity-check' });
assertGate(resolved.ok && resolved.identity !== null, 'Codex runtime identity must resolve', resolved);
const identity = resolved.identity!;
assertGate(compareSemverLike(identity.version, CURRENT_CODEX_RELEASE_MANIFEST.requiredCliVersion) >= 0, 'Codex runtime must satisfy 0.142.0', identity);
assertGate(identity.sha256.length === 64, 'Codex runtime identity must include SHA-256', identity);
emitGate('codex:0142:binary-identity', {
