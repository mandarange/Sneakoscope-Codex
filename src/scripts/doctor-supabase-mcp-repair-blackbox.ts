...
assertGate(unsafe.ok === false && unsafe.manual_required === true, 'Supabase unsafe write access must not pass silently', unsafe);
...
assertGate(optional.ok === true && optional.disabled === true, 'optional disabled Supabase config must be preserved', optional);
