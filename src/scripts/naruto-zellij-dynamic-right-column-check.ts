...
const report = {
  ...
  passes_viewport_ui: worker.includes("'headless_by_design_viewport_ui'") && runtime.includes('openHeadlessByDesignViewportWorker(paneInput)'),
  retired_escape_hatch_absent: !runtime.includes('SKS_ZELLIJ_LEGACY_WORKER_PANES') && !runtime.includes('openWorkerPane(paneInput)'),
...
report.ok = report.initial_main_only
  ...
  && report.retired_escape_hatch_absent
