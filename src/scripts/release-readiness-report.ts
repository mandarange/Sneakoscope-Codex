...
const versionTruth = {
  ...
  codex_manifest_target: String(readJson('config/codex-releases/rust-v0.144.1.json', {}).targetTag || ''),
...
})) if (!['rust-v0.144.1', '0.144.1'].includes(actual)) remainingP0.push(`codex_0144_version_mismatch:${id}:${actual || 'missing'}`);
...
const report = {
  ...
  scope: {
    ...
    ok_means: 'current 6.1 release contract, Codex 0.144.1, native capability self-repair, Voxel TriWiki, flagship routes, and signed full-run stamp have no structural blocker',
