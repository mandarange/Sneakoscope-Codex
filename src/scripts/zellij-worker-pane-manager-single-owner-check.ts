...
const nativePath = path.join(root, 'src', 'core', 'agents', 'native-cli-worker-runtime.ts')
...
const nativeUsesManager = (
  native.includes('openWorkerPane({')
  || native.includes('openHeadlessByDesignViewportWorker(')
) && native.includes('closeWorkerPane({')
...
emit({
  ...
  blockers: ok ? [] : [
    ...
    ...(nativeDirectNewPane ? ['native_cli_worker_runtime_direct_new_pane'] : []),
	    ...(!nativeUsesManager ? ['native_cli_worker_runtime_not_using_worker_pane_manager'] : []),
