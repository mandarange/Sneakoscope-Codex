...
test('codex-lb durable env file mode writes env file and reports durability', async () => {
  ...
  const result = await runProcess(process.execPath, ['./dist/bin/sks.js', 'codex-lb', 'setup', '--host', 'lb.example.test', '--api-key-stdin', '--yes', '--write-env-file', '--no-keychain', '--no-launchctl', '--json'], {
    input: 'sk-clb-env\n',
