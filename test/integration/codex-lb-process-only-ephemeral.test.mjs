...
test('codex-lb process-only setup warns and writes no env file', async () => {
  ...
  const result = await runProcess(process.execPath, [path.resolve('dist/bin/sks.js'), 'codex-lb', 'setup', '--host', 'lb.example.test', '--api-key-stdin', '--yes', '--no-env-file', '--no-keychain', '--no-launchctl', '--shell-profile', 'skip', '--json'], {
    cwd: home,
