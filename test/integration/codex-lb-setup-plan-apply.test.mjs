...
test('codex-lb setup truthfulness release check passes plan/apply modes', async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), 'sks-codex-lb-truth-runner-'));
  try {
    await fs.symlink(path.resolve('dist'), path.join(home, 'dist'), 'dir');
    const result = await runProcess(process.execPath, [path.resolve('dist/scripts/codex-lb-setup-truthfulness-check.js')], {
      cwd: home,
