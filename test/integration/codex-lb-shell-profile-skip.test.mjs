...
test('codex-lb --shell-profile skip writes no shell profile files', async () => {
  ...
  const result = await runProcess(process.execPath, [path.resolve('dist/bin/sks.js'), 'codex-lb', 'setup', '--host', 'lb.example.test', '--api-key-stdin', '--yes', '--shell-profile', 'skip', '--json'], {
    cwd: home,
    input: 'sk-clb-test\n',
