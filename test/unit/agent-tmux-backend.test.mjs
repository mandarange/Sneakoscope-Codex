...
test('tmux agent backend declares overview pane and self-closing pane policy', async () => {
  ...
  assert.equal(plan.agent_pane.self_close, true);
  assert.match(plan.agent_pane.self_close_trap, /kill-pane/);
  ...
  assert.equal(report.self_closing_panes, true);
});
