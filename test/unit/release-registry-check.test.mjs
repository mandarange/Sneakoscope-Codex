...
if (args[0] === 'pack') {
  console.log(JSON.stringify([{ name: 'sneakoscope', version: '1.15.1' }]));
  process.exit(0);
}
...
if (args[0] === 'view' && args[1] === 'sneakoscope@1.15.1') {
  console.error('No match found for version 1.15.1');
  process.exit(1);
}
...
`);
  ...
  assert.deepEqual(viewCalls.map((call) => call.args[1]), ['sneakoscope@latest', 'sneakoscope@1.15.1']);
