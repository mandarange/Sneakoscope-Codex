...
test('mixed Korean complaint plus fix/release directive routes to Naruto, not Answer', () => {
  const prompt = '아니 codex app이랑 호환이 안되는거같은데...?? 원인 분석해서 완벽하게 다음버전으로 수정하고 배포 준비해줘 use context7 mcp';
  assert.equal(routePrompt(prompt)?.id, 'Naruto');
...
test('English question-shaped bug report with explicit release work routes to Naruto', () => {
