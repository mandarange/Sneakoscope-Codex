...
test('buildScoutConsensus blocks source policy when any parse fails', () => {
  ...
  assert.equal(consensus.source_policy.mode, 'blocked_on_parse_failure');
  assert.equal(consensus.source_policy.counts.parse_failed_blocked, 1);
  assert.ok(consensus.blockers.includes('scout_output_parse_failed:invalid_json'));
});
