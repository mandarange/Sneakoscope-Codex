...
test('tmux lane content truth blocks stale capture content', async () => {
  const mod = await import('../../dist/core/agents/tmux-physical-proof.js');
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'sks-tmux-content-test-'));
  ...
  const proof = await mod.buildTmuxPhysicalProof(root, {
    ...
    captureByPaneId: { '%101': 'stale output\n' },
    writeArtifacts: false
  });
  assert.equal(proof.lane_content_truth.ok, false);
});
