...
test('authoritative skill reconciliation failure blocks doctor core readiness', () => {
  const m = buildDoctorReadinessMatrix({
    codex: { bin: '/usr/bin/codex', available: true },
    codex_config: baseConfig,
    zellij: { status: 'ok', ok: true },
    skills: {
      global: { ok: false, warnings: ['skill_target_prepare_failed'] },
      project: { ok: true, warnings: [] }
    }
  });
  assert.equal(m.core_ready, false);
  assert.equal(m.ready, false);
  assert.ok(m.blockers.includes('global_skills_reconcile_failed'));
  assert.ok(m.warnings.includes('global_skills:skill_target_prepare_failed'));
});
